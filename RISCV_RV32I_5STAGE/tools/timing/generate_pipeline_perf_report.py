...
MANAGED_PROJECTS_ROOT = PROJECT_ROOT.parent
WORKSPACE_ROOT = PROJECT_ROOT.parents[1]
AUTOMATION_ROOT = WORKSPACE_ROOT / "FPGA_Auto_Project"
if not (AUTOMATION_ROOT / "templates").exists():
    AUTOMATION_ROOT = WORKSPACE_ROOT
...
TIMING_PROFILE = "timing/timing_analysis_profile.json"
...
def run_instruction_focus_vivado(
    ...
    *,
    ...
    log_path = output_dir / "vivado_run.log"
    collector_tcl = AUTOMATION_ROOT / "templates" / "contexts" / "timing_verification" / "adapters" / "tcl" / "focus_collect_core.tcl"
...
def render_program_report_section(
    ...
    lines = [
        ...
        *recommended_actions,
        ...
        "| Evidence | Location |",
        "| --- | --- |",
        f"| Artifact root | `{display_path(output_root)}` |",
        f"| Single-cycle artifacts | `{display_path(output_root / 'single_cycle')}` |",
        f"| 5-stage artifacts | `{display_path(output_root / 'pipeline')}` |",
        f"| Instruction-focus artifacts | `{display_path(output_root / 'instruction_focus')}` |",
        f"| Program memory | `{display_path(program_selection['mem_path'])}` |",
...
def build_integrated_pipeline_detail_text(
    ...
    *,
    ...
    generated_at = datetime.now().astimezone().strftime("%Y-%m-%d %H:%M:%S %Z")
    lines = [
        f"- Source report: `{display_path(report_path)}`",
...
def main(argv: list[str] | None = None) -> None:
    pipeline_contract = load_project_contract(PROJECT_ROOT, profile_name=TIMING_PROFILE)
    ...
    single_project_root = MANAGED_PROJECTS_ROOT / single_project_name
    ...
    integrated_report_path = PROJECT_ROOT / str(pipeline_profile.get("integrated_report_path", "md/INTEGRATED_TIMING_REPORT.md"))
    ...
    collector_tcl = PROJECT_ROOT / "tools" / "timing" / "pipeline_perf_collect.tcl"
    ...
    tracker.step(
        f"Loaded pipeline timing contracts: {single_contract['project_name']} vs {pipeline_contract['project_name']} | Program: {selected_program['display_name']} | Output root: {display_path(output_root)} | Report: {display_path(report_path)}"
    )
    if focus_cfg.get("enabled", False) and not args.skip_instruction_focus:
    ...
    if reuse_baseline:
        tracker.step(f"Reused existing baseline artifacts from {display_path(output_root)}")
    ...
    if focus_cfg.get("enabled", False):
        if args.skip_instruction_focus:
            ...
            tracker.step(f"Reused existing instruction-focus artifacts from {display_path(focus_output)}")
    ...
    tracker.step(f"Report section updated for {selected_program['display_name']} at {display_path(report_path)}")
    print(f"[INFO] Wrote {display_path(report_path)}")
