...
SLAVE FPGA top  = `SortDisplaySlaveTop`
```

The current project manifest targets the MASTER top. A SLAVE build flow should select `SortDisplaySlaveTop` explicitly or use a SLAVE-specific manifest/build target.
